...
Each lost customer costs significantly in acquisition costs for replacement.
This system identifies at-risk customers BEFORE they leave,
so retention offers can be made proactively.

**Baseline:** Company offers discounts randomly to all customers (wasteful)
...
churn-predictor/
├── data/
│   ├── telco_churn.csv              ← raw data (IBM dataset)
...
Three features were created from domain knowledge:

| Feature | Logic | Why It Matters |
|---------|-------|----------------|
| `charges_per_tenure` | MonthlyCharges / (tenure+1) | New customers paying high feel ripped off |
| `is_new_customer` | tenure <= 6 | First 6 months = highest churn risk |
| `total_services` | sum of active services | More services = more locked in |
...
- Feature engineering beat raw features:
  engineered `charges_per_tenure` became #1 most important feature
- Class imbalance handling with `class_weight='balanced'`
- Production ML lifecycle: data → features → model → API → deployment